...
- [Руководство пользователя](#руководство-пользователя)
  - [Аргументы командной строки](#аргументы-командной-строки-для-mainpy)
  - [Настройка источников данных](#настройка-источников-данных)
  - [Добавление новых критериев](#добавление-нового-критерия-обнаружения)
  - [Настройка параметров обнаружения](#настройка-параметров-обнаружения)
  - [Многокритериальная валидация](#многокритериальная-валидация-кандидатов)
...
### Многокритериальная валидация кандидатов

ArcticCyclone теперь использует усовершенствованную систему многокритериальной валидации для уменьшения количества ложных срабатываний при обнаружении циклонов. Эта система оценивает каждого кандидата по нескольким критериям и принимает решение о его действительности на основе взвешенной оценки.

#### Конфигурация многокритериальной валидации

Добавьте следующую секцию в ваш файл `config.yaml` для настройки многокритериальной валидации:

```yaml
detection:
  # ... другие параметры обнаружения ...
  
  criteria:
    pressure_minimum:
      enabled: true
      weight: 0.25
      # ... другие параметры критерия ...
    vorticity:
      enabled: true
      weight: 0.30
      # ... другие параметры критерия ...
  ...
  multi_criteria_validator:
    # Порог валидации (0.0-1.0) - кандидат считается действительным, 
    # если его общий балл превышает это значение
    validation_threshold: 0.6
    
    # Расстояние для фильтрации многоканальных систем (км)
    multi_center_distance: 200.0
    
    # Пороговые значения для оценки по каждому критерию
    thresholds:
      pressure_minimum:
        very_strong: 980   # hPa
        strong: 990        # hPa
      ...
      wind_threshold:
        ...
        weak: 10             # m/s
      pressure_gradient:
        very_strong: 1.5     # hPa/100km
        strong: 1.0          # hPa/100km
        moderate: 0.7        # hPa/100km
        weak: 0.4            # hPa/100km
```

#### Как работает многокритериальная валидация

1. **Взвешенная оценка**: Каждый активный критерий оценивает кандидата по шкале от 0 до 1, где:
   - 0 = Критерий полностью не выполнен
   - 0.4-0.8 = Частично выполнен
   - 1.0 = Полностью выполнен

2. **Общий балл**: Общий балл кандидата вычисляется как взвешенная сумма индивидуальных оценок:
   ```
   Общий_балл = Σ(оценка_критерия × вес_критерия)
   ```

3. **Принятие решения**: Кандидат считается действительным циклоном, если его общий балл превышает `validation_threshold` (по умолчанию 0.6).

#### Критерии валидации

##### 1. Минимум давления (`pressure_minimum`)
Оценивает центральное давление кандидата:
- 1.0: давление ≤ `very_strong` (по умолчанию 980 гПа)
- 0.8: давление ≤ `strong` (по умолчанию 990 гПа)
- 0.6: давление ≤ `moderate` (по умолчанию 1000 гПа)
- 0.4: давление ≤ `weak` (по умолчанию 1010 гПа)
- 0.0: давление > `weak`

##### 2. Завихренность (`vorticity`)
Оценивает относительную завихренность на уровне 850 гПа:
- 1.0: |завихренность| ≥ `very_strong` (по умолчанию 5×10⁻⁵ 1/с)
- 0.8: |завихренность| ≥ `strong` (по умолчанию 3×10⁻⁵ 1/с)
- 0.6: |завихренность| ≥ `moderate` (по умолчанию 1×10⁻⁵ 1/с)
- 0.4: |завихренность| ≥ `weak` (по умолчанию 5×10⁻⁶ 1/с)
- 0.0: |завихренность| < `weak`

##### 3. Фильтр размера (`size_filter`)
Оценивает размер циклонической системы:
- 1.0: диаметр в диапазоне `optimal_min`-`optimal_max` (по умолчанию 100-800 км)
- 0.5-1.0: диаметр в диапазоне `extended_min`-`extended_max` (по умолчанию 50-1000 км)
- 0.0: диаметр вне расширенного диапазона

##### 4. Скорость ветра (`wind_threshold`)
Оценивает максимальную скорость ветра в системе:
- 1.0: скорость ≥ `very_strong` (по умолчанию 20 м/с)
- 0.8: скорость ≥ `strong` (по умолчанию 15 м/с)
- 0.6: скорость ≥ `moderate` (по умолчанию 12 м/с)
- 0.4: скорость ≥ `weak` (по умолчанию 10 м/с)
- 0.0: скорость < `weak`

##### 5. Градиент давления (`pressure_gradient`)
Оценивает максимальный градиент давления в системе:
- 1.0: градиент ≥ `very_strong` (по умолчанию 1.5 гПа/100км)
- 0.8: градиент ≥ `strong` (по умолчанию 1.0 гПа/100км)
- 0.6: градиент ≥ `moderate` (по умолчанию 0.7 гПа/100км)
- 0.4: градиент ≥ `weak` (по умолчанию 0.4 гПа/100км)
- 0.0: градиент < `weak`

#### Фильтрация многоканальных систем

Система также включает фильтрацию многоканальных систем (систем с несколькими центрами давления), которая:
1. Группирует кандидатов, находящихся ближе чем `multi_center_distance` км друг от друга
2. Выбирает наилучшего кандидата из каждой группы на основе комбинации давления, завихренности и скорости ветра
3. Удаляет остальные кандидаты из группы

Это значительно снижает количество ложных срабатываний от сложных метеорологических систем с множественными центрами.
